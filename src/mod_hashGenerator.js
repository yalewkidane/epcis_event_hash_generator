...
const jsonToNode = require('./jsonToNode').jsonToPy;
...
function _fix_time_stamp_format(timestamp) {
    console.log(`correcting timestamp format for '${timestamp}'`);
    ...
    try {
        ...
        console.log(`'${timestamp}' is labelled as time but does not match the ISO 8601 dateTime format`);
        return timestamp;
    }
}
...
function _recurse_through_children_in_order(child_list, child_order) {
    ...
    */
    ...
    for (let [child_name, sub_child_order] of child_order.entries()) {
...
function _gather_user_extensions(childList) {
    ...
    for (let child of childList) {
        if (child[0].includes('eventTime') || child[0].includes('action')) {
            return userExtensions;
        }
    }
    ...
    for (let x of childList) {
        if (typeof x === 'object' && x[0].includes('{') && x[0].includes('/}')) {
...
function _generic_child_list_to_prehash_string(children) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (Array.isArray(child) && child.length == 2 && Array.isArray(child[0])) {
            ...
            //console.log("-----------------===============", child[1])
            let text = child[1].trim();
            if (text) {
                text = _canonize_value(text);
                text = "=" + text;
            }

            list_of_values.push(child[0] + text + _generic_child_list_to_prehash_string(child[2]));
        }
    }
...
function _gather_elements_not_in_order(children, child_order) {
    ...
    */
    ...
    children = children.filter(child => !to_be_ignored.includes(child[0]));
    if (children.length) {
